...
from AnanyaMusic.core.call import Aviax as Shashank
...
from AnanyaMusic.utils.pastebin import AviaxBin as ShashankBin
from AnanyaMusic.utils.stream.queue import put_queue, put_queue_index
from AnanyaMusic.utils.thumbnails import gen_thumb as get_thumb
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await Shashank.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Shashank.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Shashank.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
